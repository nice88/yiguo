<template>
	...
	<div class="fruit" v-else>
		...
		<div class="box">
			<div class="pro" v-for="item in datas" :key="item.id">
				<div class="img" @click="to(item.id)"><img :src="item.goods_img"></div>
				<div class="detail">
					<h3>{{item.name}}</h3>
					<p>{{item.detail_name}}</p>
					<i class="reduce">{{item.pro_addr}}</i>
					<p class="price"><span class="money">¥{{item.price}}</span><del>¥{{item.marketprice}}</del></p>
...
export default{
	...
	methods: {
	    handlePage(index){
      		...
      		this.axios.get('http://121.199.63.71:8003/type/list/child/'+this.$route.query.id+'/'+this.activeClass+'/').then((res)=>{
						var msg = res.data.msg;
						if (msg === 'ok') {
							this.datas = res.data.type_detail;
						}
					})
      	},
      	handleScroll() {
	...
	mounted() {
	    ...
	    this.axios.get('http://121.199.63.71:8003/type/list/child/'+this.$route.query.id+'/0/').then((res)=>{
				var msg = res.data.msg;
				if (msg === 'ok') {
					this.show = false;
					this.datas = res.data.type_detail;
