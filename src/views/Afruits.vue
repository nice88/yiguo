...
export default{
	...
	methods: {
	    handlePage(index){
      		...
      		this.axios.get('http://121.199.63.71:8003/home/nav/'+this.$route.query.id+'/'+this.activeClass+'/').then((res)=>{
	...
	mounted() {
	    ...
	    this.axios.get('http://121.199.63.71:8003/home/nav/'+this.$route.query.id+'/0/').then((res)=>{
				var msg = res.data.msg;
				if (msg === 'ok') {
					this.show = false;
					this.datas = res.data.datas;
